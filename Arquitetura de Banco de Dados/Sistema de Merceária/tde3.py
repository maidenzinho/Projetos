...
from PyQt5.QtWidgets import (
    ...
    QMessageBox, QHBoxLayout, QLabel, QInputDialog
)
from sqlalchemy import create_engine, Column, Integer, String, Float, ForeignKey
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import sessionmaker
from sqlalchemy.exc import SQLAlchemyError  # Importando SQLAlchemyError
...
# Controle de acesso baseado em permissões de usuários
PERMISSOES = {
    "admin": ["ler", "escrever", "deletar"],
    "gerente": ["ler", "escrever"],
    "funcionario": ["ler"]
}
usuario_atual = {"nome": "admin", "nivel": "admin"}  # Alterar conforme o usuário logado

def verificar_permissao(acao):
    if acao not in PERMISSOES[usuario_atual["nivel"]]:
        QMessageBox.warning(None, "Acesso Negado", f"Você não tem permissão para realizar a ação: {acao}.")
        return False
    return True

# Consultas avançadas com álgebra relacional
def consulta_cliente_produto(valor_minimo):
    try:
        clientes = sessao.query(Cliente.nome, Produto.nome).join(Venda, Cliente.id == Venda.clienteid).join(
            Produto, Venda.produtoid == Produto.id).filter(Produto.preco > valor_minimo).all()
        return clientes
    except SQLAlchemyError as e:
        QMessageBox.critical(None, 'Erro', f"Erro ao executar consulta: {e}")
        return []
...
class MainWindow(QMainWindow):
        ...
        if current_row >= 0:
            ...
            if novo_nome:
                ...
                try:
                    ...
                    QMessageBox.critical(self, 'Erro', f"Erro ao atualizar categoria: {e}")
            else:
                QMessageBox.warning(self, 'Erro', 'O nome não pode ser vazio.')
        else:
    ...
    def deletar_categoria(self):
        ...
        if current_row >= 0:
            ...
            try:
                ...
                QMessageBox.critical(self, 'Erro', f"Erro ao deletar categoria: {e}")
        else:
    ...
    def adicionar_produto(self):
        ...
        if nome and preco and estoque and categoriaid:
            ...
            try:
                ...
                QMessageBox.critical(self, 'Erro', f"Erro ao adicionar produto: {e}")
        else:
    ...
    def deletar_cliente(self):
        ...
        if current_row >= 0:
            ...
            try:
                ...
                QMessageBox.critical(self, 'Erro', f"Erro ao deletar cliente: {e}")
        else:
    ...
    def adicionar_venda(self):
        ...
        if produtoid and clienteid and quantidade:
            ...
            try:
                ...
                QMessageBox.critical(self, 'Erro', f"Erro ao adicionar venda: {e}")
        else:
            QMessageBox.warning(self, 'Erro', 'Todos os campos devem ser preenchidos.')
        ...
        if current_row >= 0:
            ...
            if novo_produtoid and novo_clienteid and nova_quantidade:
                ...
                try:
                    ...
                    QMessageBox.critical(self, 'Erro', f"Erro ao atualizar venda: {e}")
            else:
                QMessageBox.warning(self, 'Erro', 'Todos os campos devem ser preenchidos.')
        else:
    ...
    def deletar_venda(self):
        ...
        if current_row >= 0:
            ...
            try:
                ...
                QMessageBox.critical(self, 'Erro', f"Erro ao deletar venda: {e}")
        else:
    ...
    def relacional_ui(self):
        self.relacional_label = QLabel("Consultas Avançadas com Álgebra Relacional")
        self.layout.addWidget(self.relacional_label)

        self.busca_cliente_produto_button = QPushButton("Clientes que compraram produtos acima de X valor")
        self.busca_cliente_produto_button.clicked.connect(self.executar_consulta_cliente_produto)
        self.layout.addWidget(self.busca_cliente_produto_button)

    def executar_consulta_cliente_produto(self):
        if not verificar_permissao("ler"):
            return
        valor_minimo, ok = QInputDialog.getDouble(self, "Valor Mínimo", "Digite o valor mínimo do produto:")
        if ok:
            clientes = consulta_cliente_produto(valor_minimo)
            if clientes:
                resultado = "\n".join([f"{cliente[0]} comprou {cliente[1]}" for cliente in clientes])
                QMessageBox.information(self, "Resultado", resultado)
            else:
                QMessageBox.information(self, "Resultado", "Nenhum cliente encontrado para o critério.")
